...
static const struct state *disabled_impl(struct statemachine *sm, const struct event *ev)
{
    switch (ev->id)
    {
        case EV_ENTRY: {
            //statemachine_subscribe_do(sm);
            printf("%s(1) -%s%s%s, var1=%d, var2=%d\n", __func__, EVENTCOL2, ENUM2STRING(ev->id), NORM, var1, var2);
            return statemachine_event_handled();
        }
        ...
        case EV_DO: {
            if (flag1 == 1)
	    {
                printf("%s(2) -%s%s%s, flag1=%d\n", __func__, EVENTCOL2, ENUM2STRING(ev->id), NORM, flag1);
	        return &enabled;
	    }

            printf("%s() -%s%s%s, flag1=%d\n", __func__, EVENTCOL, ENUM2STRING(ev->id), NORM, flag1);
            return statemachine_event_handled();
	    //break;
	}

        case EV_EXIT: {
            printf("%s(3) -%s%s%s, flag1=%d\n", __func__, EVENTCOL2, ENUM2STRING(ev->id), NORM, flag1);
	    var1 = 30;
	    var2 = 20;
            return statemachine_event_handled();
        }
...
static const struct state *enabled_impl(struct statemachine *sm, const struct event *ev)
{
    switch (ev->id) 
    {
        ...
        case EV_DO: {
            if (flag1 == 0)
	    {
                printf("%s(3) -%s%s%s, flag1=%d\n", __func__, EVENTCOL3, ENUM2STRING(ev->id), NORM, flag1);
                return &disabled;
	    }
	    break;
	}
...
static const struct state *idle_impl(struct statemachine *sm, const struct event *ev)
{
    switch (ev->id) {
        ...
        case EV_DO: {
            ...
            if (flag2 != 0)
            {
               printf("%s(3) -%s%s%s, flag2=%d\n", __func__, EVENTCOL5, ENUM2STRING(ev->id), NORM, flag2);
	       return &running; 
            }

	    else if (flag2 == 0)
	    {
               printf("%s(4) -%s%s%s, flag2=%d\n", __func__, EVENTCOL5, ENUM2STRING(ev->id), NORM, flag2);
	       var1 = 99;
	       flag1 = 0;
	       return &disabled; 
	    }
